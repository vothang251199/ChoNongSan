	...
	public class UserController : Controller
	{
		...
		public IActionResult ResetPassword(string tokensEmail)
		{
			try
			{
				...
				var email = decoded[1];
				var username = decoded[2];
				string date = decoded[3];
				string[] time = decoded[4].Split("-");
				if ((Convert.ToUInt32(DateTime.Now.ToString("mm")) - Convert.ToUInt32(time[1])) <= 5 && date.Equals(DateTime.Now.ToString("dd/MM/yyyy")))
				{
				...
				{
					TempData["ALertMessage"] = "Url đã hết hạn";
